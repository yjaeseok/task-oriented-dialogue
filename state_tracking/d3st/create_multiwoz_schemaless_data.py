...
def create_schemaless_data(json_data: Json, schema_info: SchemaInfo,
    ...
    def _multiple_choice_answer(slot_id: int, letters: List[str],
        ...
        if value not in possible_values_shuffled:
            print(f'Value {value} not in possible values {possible_values_shuffled}')
...
def main():
    ...
    multiwoz_data = multiwoz_utils.load_data(
        data_path="../testdata/MULTIWOZ2.1",
        multiwoz_version="2.1",
        is_trade=False)
    schema_info = multiwoz_utils.load_schema("../testdata/multiwoz_schema_schemaless.json")
    options = Options(
        multiwoz_version="2.1",
    ...
    for split, examples in split_to_examples.items():
        with open(f'../../output/{split}.json', "w") as f:
